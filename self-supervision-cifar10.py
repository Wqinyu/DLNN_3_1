...
def byol_loss(out_1, out_2):
    return 2 - 2 * nn.functional.cosine_similarity(out_1, out_2).mean()
...
def train_byol(model, train_loader, epochs, learning_rate, target_update_rate, writer, model_path, patience=10):
    ...
    for epoch in range(epochs):
        ...
        for images, _ in train_loader:
            images = images.cuda()  # 确保输入数据在GPU上
...
def evaluate_linear_classifier(linear_model, train_loader, test_loader, epochs, learning_rate, writer, model_path, patience=10):
    linear_model = linear_model.cuda()  # 确保模型在GPU上
...
# 创建数据加载器
def create_data_loaders(batch_size, subset_ratio):
...
if __name__ == '__main__':
    # 运行实验
    learning_rate = 1e-4
    batch_size = 64  # 更小的批量大小以减少内存占用
    subset_ratio = 0.5  # 更小的数据集子集比例以减少内存占用

    # 创建数据加载器
    train_loader, train_loader_cifar100, test_loader_cifar100 = create_data_loaders(batch_size=batch_size, subset_ratio=subset_ratio)

    # 训练BYOL模型
    model = BYOL(models.resnet18, out_dim=512)
    writer = SummaryWriter(log_dir=f'runs/cifar10_byol_lr{learning_rate}_bs{batch_size}_sr{subset_ratio}')
    train_byol(model, train_loader, epochs=100, learning_rate=learning_rate, target_update_rate=0.996, writer=writer,
               model_path=f'byol_cifar10_resnet18_lr{learning_rate}_bs{batch_size}_sr{subset_ratio}.pth', patience=5)
    writer.close()
    ...
    writer = SummaryWriter(log_dir=f'runs/cifar10_linear_lr{learning_rate}_bs{batch_size}_sr{subset_ratio}')
    best_accuracy = evaluate_linear_classifier(linear_model, train_loader_cifar100, test_loader_cifar100,
                                               epochs=50, learning_rate=learning_rate, writer=writer,
                                               model_path=f'cifar10_linear_cifar100_resnet18_lr{learning_rate}_bs{batch_size}_sr{subset_ratio}.pth',
                                               patience=7)
